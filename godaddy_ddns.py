...
env_api_key = os.environ.get("GODADDY_API_KEY", "")
env_api_secret = os.environ.get("GODADDY_API_SECRET", "")
env_host_name = os.environ.get("GODADDY_DOMAIN", "")
env_ttl = os.environ.get("GODADDY_TTL", 3600)

parser = argparse.ArgumentParser(
    description='Update GoDaddy DNS "A" Record.',
    fromfile_prefix_chars="%",
    epilog="""GoDaddy customers can obtain values for the KEY and SECRET arguments by creating a production key at
https://developer.godaddy.com/keys/.
...
parser.add_argument(
    "--key", type=str, default=env_api_key, help="GoDaddy production key"
)

parser.add_argument(
    "--secret", type=str, default=env_api_secret, help="GoDaddy production secret"
)

parser.add_argument("--ttl", type=int, default=env_ttl, help="DNS TTL.")
...
def main():
    ...
    if not args.ip:
        try:
            with urlopen(
                Request(
                    "https://checkip.amazonaws.com/", headers={"User-Agent": "Mozilla"}
                )
            ) as f:
                resp = f.read()
            if sys.version_info > (3,):
    ...
    for ipsiter in ipslist:
        ...
        if (
            len(ips) != 4
            or not ips[0].isdigit()
            or not ips[1].isdigit()
            or not ips[2].isdigit()
            or not ips[3].isdigit()
            or int(ips[0]) > 255
            or int(ips[1]) > 255
            or int(ips[2]) > 255
            or int(ips[3]) > 255
    ...
    data = json.dumps(
        [
            {"data": ip, "ttl": args.ttl, "name": hostnames[0], "type": "A"}
            for ip in ipslist
        ]
    )
    if sys.version_info > (3,):
    ...
    try:
        ...
        if e.code == 400:
            msg = (
                "Unable to set IP address: GoDaddy API URL ({}) was malformed.".format(
                    req.full_url
                )
            )
        elif e.code == 401:
